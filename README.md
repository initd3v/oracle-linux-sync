...
* oracle-linux-sync.sh -d=[download path] -u=[download url] [-m=[createrepo_c binary path] -l=[rate_with_unit] -v=[verbosity]]
...
| -l \| --limit-rate   | limit rate in curl download of rpm packages. Please refer to option '--limit-rate' in curl manual for details on allowed syntax | optional | STRING | -  |
